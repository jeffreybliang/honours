...
\begin{ArticleSummary}{DRWR: A Differentiable Renderer without Rendering for Unsupervised 3D Strucutre Learning }
    ...
    \textbf{Summary:} A unsupervised differentiable renderer using losses in a clever way as opposed to pixel-wise losses.
    
    \textbf{Key Points:}
    \begin{itemize}
        \item This is unsupervised, so only requires images for training.
        \item Existing pixel-wise loss functions render images from 3D structures 
    \end{itemize}
\end{ArticleSummary}
\end{document}