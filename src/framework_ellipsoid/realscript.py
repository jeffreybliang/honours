...
AXES_PRESETS_02 = {
    ...
    "A6": [0.14938, 0.29876, 0.44814]           # triaxial 1:2:3
}

AXES_PRESETS_24 = {
    "A1": [0.620, 0.620, 0.620],
    "A2": [0.493, 0.493, 0.986],
    "A3": [0.391, 0.783, 0.783],
        ...
        for a_id, axes in axes_presets.items():
            for r_id, angles in angle_presets.items():
                ...
                if problem == "chamferboundary":
                    # A3, A4, A6 → all rotations
                    if a_id in {"A3", "A4", "A6"}:
                        resume = True
                    # A5 R1 or R2 only
                    elif a_id == "A5" and r_id in {"R1", "R2"}:
                        resume = True
                elif problem == "volumeconstrained":
                    resume = True
                ...
                for trial in trial_range:
                    ...
                    # vis frequency & switch off after 5 runs
                    cfg["vis"]["enabled"] = trial < 2

                    # Optionally write the config for reproducibility
